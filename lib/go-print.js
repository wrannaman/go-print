...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'go-print:add-printf': () => this.addPrintf()
    }));
  },
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      if (selectedText.length > 0) {
        ...
        if (conditionalFlag) {
          ...
          editor.moveUp(1)
          if (styles.length > 0) {
            editor.insertText(`fmt.Printf("%c${identifier} \\n", "${styles}", ${selectedTextInsert})`)
          } else {
            editor.insertText(`fmt.Printf("${identifier} %v\\n", ${selectedTextInsert})`)
          }
          editor.moveToBeginningOfLine()
          editor.selectToEndOfLine()
          ...
          editor.insertNewline()
          if (styles.length > 0) {
            editor.insertText(`fmt.Printf("%cCONDITION PASSED\\n", '${styles}')"`)
          } else {
            editor.insertText(`fmt.Printf("CONDITION PASSED\\n")`)
          }
        } else {
          editor.moveToEndOfLine()
          editor.insertNewline()
